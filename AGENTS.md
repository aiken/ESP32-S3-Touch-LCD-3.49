...
| `ui_calendar` | Used | All screens: status bar (date/weekday/**kid label — tap to switch 美熹/壮壮**/SSID/battery+charging bolt), course timeline, month calendar, reminders, styles, CJK fonts (custom NotoSansSC **+ Source Han fallback chain** for arbitrary course-name chars), `ui_set_orientation()` for portrait/landscape rebuild, plus `ui_screenshot.c` |
